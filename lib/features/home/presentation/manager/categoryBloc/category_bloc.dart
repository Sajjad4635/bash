...
class CategoryBloc extends Bloc<CategoryEvent, CategoryState> {
  ...
  final CartInquiryUseCase cartInquiryCUseCase;

  CategoryBloc({
    required this.getCategoryList,
    required this.cartInquiryCUseCase,
  }) : super(Empty()) {
    ...
    on<GetCartInquiryCEvent>((event, emit) async {
      emit(Empty());
      emit(GetCartInquiryCLoading());
      final either = await cartInquiryCUseCase(NoParams());
      either.fold((l) {
        handleFailure(l, emit);
      }, (r) {
        emit(
          ///we can use data from 'r'
          CartInquiryCReceived(cartInquiry: r),
        );
      });
    });
  }
